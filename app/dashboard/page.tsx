...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    sb.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      setAuthUser(user);
      fetch(`/api/user?userId=${user.id}`).then(r => r.json()).then(setUser);
  ...
  const creditsTotal = isPro ? (user?.plan === "starter" ? 100 : user?.plan === "pro" ? 300 : 1000) : 1;
    ...
    <div className="p-6 max-w-4xl mx-auto" style={{ fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {showBanner && (
        <div className="relative bg-gradient-to-r from-amber-500/20 via-orange-500/10 to-amber-500/5 border border-amber-500/30 rounded-2xl p-5 mb-6 overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <button onClick={dismissBanner} className="text-slate-600 hover:text-slate-400 text-xs flex-shrink-0 transition-colors">✕ Dismiss</button>
          </div>
        </div>
      )}
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Link href="/dashboard/generate"
            ...
            <span className="inline-flex items-center gap-1 text-amber-400 text-xs font-semibold mt-3">
              Mulai Generate →
            </span>
          </Link>
          ...
          <Link href="/dashboard/bulk"
            className={`group bg-slate-900/40 border rounded-2xl p-6 transition-all hover:-translate-y-0.5 ${
              isPro
                ? "border-slate-800 hover:border-blue-500/40 hover:shadow-xl hover:shadow-blue-500/5"
                ...
                <div key={a.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-900/40 transition-colors group">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                      ...
                      <span className="text-[11px] text-slate-500">{a.word_count?.toLocaleString() || 0} kata</span>
                      <span className="text-[11px] text-slate-600">·</span>
                      <span className="text-[11px] text-slate-500">{new Date(a.created_at).toLocaleDateString("id-ID")}</span>
                    </div>
                  </div>
                  <Link href={`/documents?highlight=${a.id}`}
      ...
      <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {!isPro && (
            <Link href="/pricing"
              className="bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold text-sm px-4 py-2 rounded-xl transition-all hover:shadow-lg hover:shadow-amber-500/25">
              Upgrade
            </Link>
          )}
        </div>
