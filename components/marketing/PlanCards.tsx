  ...
  return (
    <div className="grid gap-px overflow-hidden rounded-2xl border border-stone-200 bg-stone-100 lg:grid-cols-3">
      {DISPLAY_PLANS.map((plan) => (
        <article key={plan.id} className={`relative flex min-h-[390px] flex-col bg-white p-6 sm:p-8 ${plan.featured ? "shadow-[inset_0_3px_0_#10b981]" : ""}`}>
          {plan.featured && <span className="mb-6 w-fit border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-1 text-xs font-black uppercase tracking-[0.18em] text-emerald-800">Direkomendasikan</span>}
          <p className="text-sm font-bold text-slate-400">{plan.name}</p>
          <div className="mt-4 flex items-end gap-2"><strong className="text-3xl text-slate-900 sm:text-4xl">{plan.price}</strong><span className="pb-1 text-xs text-slate-500">/ 30 hari</span></div>
          <div className="mt-7 border-y border-stone-200 py-5">
            <p className="text-lg font-bold text-emerald-800">{plan.quota}</p>
          ...
          {onBuy ? (
            <button onClick={() => onBuy(plan.id)} disabled={loading === plan.id} className={`mt-auto w-full rounded-lg px-4 py-3 text-sm font-black transition focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:opacity-60 ${plan.featured ? "bg-emerald-400 text-[#0a101b] hover:bg-emerald-300" : "border border-stone-300 text-slate-900 hover:border-emerald-400/50 hover:bg-emerald-400/5"}`}>
              {loading === plan.id ? "Memproses..." : `Pilih ${plan.name}`}
            </button>
          ) : (
            <Link href="/pricing" className={`mt-auto block rounded-lg px-4 py-3 text-center text-sm font-black transition focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 ${plan.featured ? "bg-emerald-400 text-[#0a101b] hover:bg-emerald-300" : "border border-stone-300 text-slate-900 hover:border-emerald-400/50"}`}>Lihat {plan.name}</Link>
          )}
        </article>
      ))}
    </div>
