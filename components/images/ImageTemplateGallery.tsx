...
export default function ImageTemplateGallery({
  ...
  return (
    <section className={`overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/60 ${className}`}>
      <div className="flex flex-col gap-1 border-b border-slate-800 bg-slate-900/80 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-display text-sm font-black tracking-wide text-white">Gambar Unggulan</h2>
          <p className="mt-1 text-xs text-slate-400">Enam gaya ilustrasi nonfiguratif · 1200 × 630</p>
        </div>
        <span className="mt-2 w-fit rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-emerald-400 sm:mt-0">
          Tanpa AI
        </span>
      </div>
      ...
      <div className="grid gap-5 p-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,.55fr)] lg:p-5">
        <div className="min-w-0">
          <div className="overflow-hidden rounded-xl border border-slate-700/70 bg-slate-900 shadow-2xl shadow-black/20">
            <img
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            {onUse ? (
              ...
              <span className="shrink-0 rounded-lg border border-slate-700 px-3 py-2 text-xs text-slate-400">Preview desain</span>
            )}
          </div>
        </div>
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between gap-3">
              <label htmlFor="image-template-title" className="text-xs font-bold uppercase tracking-widest text-slate-400">
                Judul Gambar
              </label>
              <span className="text-xs tabular-nums text-slate-400">{imageTitle.length}/180</span>
            </div>
            <textarea
              ...
              className="w-full resize-none rounded-lg border border-slate-700/60 bg-slate-900 px-3 py-2.5 text-xs leading-relaxed text-slate-200 outline-none transition-colors placeholder:text-slate-500 focus:border-amber-500/60"
            />
          </div>

          <fieldset>
            <legend className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-400">Warna Aksen</legend>
          ...
          <div>
            <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-400">Gaya Ilustrasi</p>
              ...
              {IMAGE_TEMPLATE_PRESETS.map(preset => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span className={`block truncate px-2 py-1.5 text-xs font-bold ${selected ? "text-amber-400" : "text-slate-400 group-hover:text-slate-200"}`}>
                      {preset.name}
                    </span>
                  </button>
                );
  ...
  return (
    <button
      ...
      className={`flex h-8 items-center gap-1.5 rounded-full border px-2 transition-all ${
        selected ? "border-slate-400 bg-slate-800" : "border-slate-800 bg-slate-900 hover:border-slate-700"
      }`}
    >
      <span
      ...
      />
      <span className={`text-xs ${selected ? "text-slate-200" : "text-slate-400"}`}>{palette.name}</span>
    </button>
  );
}
