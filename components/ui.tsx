...
export const Tog = ({ label, val, set, disabled }: { label: string; val: boolean; set: (v: boolean) => void; disabled?: boolean }) => (
  <div className={`flex items-center justify-between gap-2 py-1 ${disabled ? "opacity-40" : ""}`}>
    <span className="text-sm text-slate-700 leading-tight">{label}</span>
    <button type="button" onClick={() => !disabled && set(!val)} aria-pressed={val} className={`relative w-10 h-5 rounded-full flex-shrink-0 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2 ${val ? "bg-amber-500" : "bg-stone-300"}`}>
      <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-200 ${val ? "left-[21px]" : "left-0.5"}`} />
    </button>
  </div>
);
...
export const Sel = ({ label, opts, val, set, tooltip }: { label?: string; opts: string[]; val: string; set: (v: string) => void; tooltip?: string }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <select value={val} onChange={e => set(e.target.value)} className="h-10 w-full cursor-pointer rounded-lg border border-stone-300 bg-white px-3 text-sm text-slate-800 outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100">
...
export const Sec = ({ title, icon, children, collapsible = true, defaultOpen }: { title: string; icon: string; children: ReactNode; collapsible?: boolean; defaultOpen?: boolean }) => {
  ...
  if (collapsible && !open) return (
    <button type="button" onClick={() => setOpen(true)} className="w-full flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-left shadow-sm transition hover:border-amber-200 hover:bg-amber-50/40 focus:outline-none focus:ring-2 focus:ring-amber-300">
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
      <button type="button" onClick={() => collapsible && setOpen(false)} className={`w-full flex items-center justify-between px-4 py-3.5 text-left ${collapsible ? "hover:bg-stone-50 cursor-pointer" : "cursor-default"} transition-colors`}>
...
export const Badge = ({ text, v = "default" }: { text: string; v?: string }) => {
  ...
  return <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${s[v] || s[text] || s.default}`}>{text}</span>;
};
