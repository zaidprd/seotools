    ...
    <div className="flex flex-col gap-2.5">
      ...
      {sites.length > 0 && (
        <div className="flex flex-col gap-1.5">
          ...
          <button onClick={() => setShowForm(!showForm)} className="text-xs text-emerald-500 hover:text-emerald-700 text-center py-1 transition-colors">+ Tambah situs lain</button>
        </div>
      )}
      {showForm && (
        <div className="bg-white border border-stone-300 rounded-xl p-3.5 flex flex-col gap-2.5">
          ...
          <div className="bg-emerald-500/8 border border-emerald-500/20 rounded-lg px-3 py-2 text-xs text-emerald-800 leading-relaxed">💡 WP Admin → Users → Profile → <strong>Application Passwords</strong></div>
          {[{ k: "name", l: "Nama Situs", p: "cth: Blog Utama" }, { k: "url", l: "URL WordPress", p: "https://situskamu.com" }, { k: "user", l: "Username", p: "admin" }, { k: "pass", l: "Application Password", p: "xxxx xxxx xxxx xxxx", t: "password" }].map(({ k, l, p, t }) => (
            <div key={k} className="flex flex-col gap-1">
              ...
              <input type={t || "text"} value={(f as any)[k]} onChange={e => setF(x => ({ ...x, [k]: e.target.value }))} placeholder={p} className="bg-white border border-stone-300 text-slate-800 text-xs rounded-lg px-3 py-2 focus:outline-none focus:border-emerald-500/60 placeholder-stone-400" />
            </div>
          ))}
          <div className="flex gap-2 mt-0.5">
            ...
            <button onClick={save} className="flex-1 text-xs bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold py-2 rounded-lg transition-colors">Simpan</button>
          </div>
