...
function FetchSitemap({ baseUrl, onFetch }: { baseUrl: string; onFetch: (pages: string) => void }) {
  ...
  return (
    <div className="border-t border-stone-200 pt-2">
      <p className="text-sm font-semibold text-slate-700 mb-1.5">Fetch dari Sitemap</p>
  ...
  return (
    <div className="flex flex-col gap-1.5 pb-6 pr-0.5">
      ...
      <Sec title="Gaya & audiens" icon="Aa" defaultOpen={false}>
        ...
        <div className="border-t border-stone-200 pt-2.5 mt-0.5">
          <p className="text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Gaya penulisan</p>
          <div className="grid grid-cols-2 gap-2">
        ...
        <div className="border-t border-stone-200 pt-2.5 mt-0.5 flex items-start gap-2">
          <span className="text-emerald-700">✓</span>
          <div>
            <p className="text-xs font-bold text-slate-400">Konfigurasi penulisan dipilih otomatis</p>
            <p className="text-xs text-slate-500">Dioptimalkan untuk artikel panjang, natural, dan konsisten.</p>
          </div>
        </div>
      </Sec>
      ...
      <Sec title="Suara merek" icon="✎" defaultOpen={false}>
        <p className="text-xs text-slate-500 leading-relaxed">Buat gaya unik agar konten selalu konsisten.</p>
      ...
      <Sec title="Featured image" icon="▧" defaultOpen={false}>
        <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 text-xs leading-relaxed text-emerald-800">
        ...
        <div className="flex flex-col gap-2">
          ...
          {cfg.internalLinkSite !== "Tidak Ada" && (
            <div className="flex flex-col gap-2 mt-1">
              {/* Base URL */}
              <div>
                <label className="text-sm font-semibold text-slate-700 block mb-1">Base URL Website</label>
                <input
              ...
              <div>
                <label className="text-sm font-semibold text-slate-700 block mb-1">Daftar Halaman (opsional)</label>
                <textarea
        ...
        <div className="flex flex-col gap-2">
          ...
          {cfg.extLinkType === "Otomatis" && (
            <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 px-3 py-2">
              <p className="text-xs leading-relaxed text-blue-300">Mencari maksimal 3 halaman nyata melalui Wikipedia Indonesia. Jika tidak ada hasil yang relevan, artikel dibuat tanpa external link.</p>
            </div>
          )}
          {cfg.extLinkType === "Manual" && (
            <div>
              <label className="text-sm font-semibold text-slate-700 block mb-1">URL External</label>
              <textarea
      ...
      <Sec title="WordPress" icon="W" defaultOpen={false}>
        ...
        {wpSel && (
          <div className="mt-2 pt-2 border-t border-stone-200 flex flex-col gap-2">
